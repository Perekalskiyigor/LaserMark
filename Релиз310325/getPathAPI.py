...
def fetch_orders_from_db(OrderID):
    try:
        ...
        sql_query = "SELECT id, dataAdd, id_module, templates_for_marking, OrderID, templates_for_markingFront FROM Orders WHERE OrderID = ? ORDER BY dataAdd DESC LIMIT 1"
...
def getPathTemplate(orders):
    if orders:
        for order in orders:      
            ...
            OrderID = order[4]
            idTemplate = order[0]
            return path, OrderID, idTemplate, pathFront
        else:
            logging.info(f"getPathAPI - NO path Template")
            return ""
        
# 4. Функция для копирования шаблона в папку Templates
def save_template_to_project_folder(template_path, OrderID, idTemplate, pathFront):
    if template_path and OrderID:
        ...
        if pathFront:
            ...
            try:
                shutil.copy(pathFront, front_file_destination)
                logging.info(f"getPathAPI - Front file successfully copied to {front_file_destination}")
            except FileNotFoundError:
                logging.error(f"getPathAPI - Error: front file not found at {pathFront}")
            except Exception as e:
