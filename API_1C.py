...
def process_order_data(order_for_production_code, order_for_production_year):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            VALUES (?, ?, ?, ?, ?, ?) 
            ''', (dataAdd, module, catalog_number, str(templates_for_marking), str(len(serial_numbers)), order_for_production_code))
    ...
    # Save to folder module getPathAPI
    try:
        status = getPathAPI.process_templates(module)
        logging.info(f"Processing status: {status}")
    except Exception as e:
        logging.error(f"Error while processing templates: {e}")
