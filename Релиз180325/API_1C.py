...
def process_order_data(order_for_production_code, order_for_production_year):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            ######################## Шаблоны для маркировкиФРОНТ ############################
            templates_for_markingFront = data.get('templates_for_marking', [])
            second_path = templates_for_markingFront[1]['path']
            ...
            cursor.execute(''' 
            INSERT INTO Orders (dataAdd, id_module, catalog_number, templates_for_marking, templates_for_markingFront, serial_numbers, OrderID) 
            VALUES (?, ?, ?, ?, ?, ?, ?) 
            ''', (dataAdd, module, catalog_number, str(first_path),  str(second_path), str(len(serial_numbers)), order_for_production_code))
