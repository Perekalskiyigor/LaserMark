...
def cutting_process(new_value):
    ...
    for i in range(1, 11):  # Цикл из 10 итераций
        while True:  # Ожидаем данных, не пропуская итерации
            ...
            #print("Ответ от сервера:", decoded_data)
            response = "Python"
            if "Python" in response:  # Если данные получены, выходим из ожидания
                break
            print("Ждем данные...")
            time.sleep(1)  # Ожидание перед повторной попыткой

        time.sleep(5)  # Ожидание перед выполнением действий
...
def update_message(text):
    ...
    '''
        ...
        some_data = str.encode(f"Id.Data=888", encoding='UTF-8')
        win32file.WriteFile(pipe, some_data)
        message.config(text="")  # Устанавливаем пустую строку, чтобы очистить текущий текст
        message.config(text=f"Установка 888 с паузой")
        time.sleep(7)  # Задержка 1 секунда
        # Возвращаем обратно
        some_data = str.encode(f"Set new Value", encoding='UTF-8')
        win32file.WriteFile(pipe, some_data)
        time.sleep(1)  # Задержка 1 секунда
        some_data = str.encode(f"Id.Data=000", encoding='UTF-8')
        win32file.WriteFile(pipe, some_data)
        message.config(text="")  # Устанавливаем пустую строку, чтобы очистить текущий текст
        message.config(text=f"Установка 000 с паузой")
